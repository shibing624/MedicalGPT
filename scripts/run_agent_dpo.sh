# Agent DPO: tool call preference optimization
# Uses ShareGPT format with conversations/tools/chosen/rejected fields
# --tool_format controls how function_call / observation roles are formatted
# Can also be mixed with normal DPO data (alpaca format) by placing both
# file types in the same train_file_dir folder

CUDA_VISIBLE_DEVICES=0,1 python training/dpo_training.py \
    --model_name_or_path Qwen/Qwen3.5-2B \
    --train_file_dir ./data/toolcall \
    --validation_file_dir ./data/toolcall \
    --per_device_train_batch_size 4 \
    --gradient_accumulation_steps 8 \
    --per_device_eval_batch_size 1 \
    --do_train \
    --do_eval \
    --use_peft True \
    --max_train_samples 1000 \
    --max_eval_samples 10 \
    --max_steps 100 \
    --eval_steps 20 \
    --save_steps 50 \
    --max_source_length 1024 \
    --max_target_length 512 \
    --output_dir outputs-agent-dpo-v1 \
    --target_modules all \
    --lora_rank 8 \
    --lora_alpha 16 \
    --lora_dropout 0.05 \
    --torch_dtype bfloat16 \
    --bf16 True \
    --fp16 False \
    --report_to tensorboard \
    --remove_unused_columns False \
    --gradient_checkpointing True \
    --tool_format default \
    --cache_dir ./cache
